...
WITH cte as(
    SELECT c.customer_id, c.name, o.order_id, o.order_date, RANK() OVER(PARTITION BY o.customer_id ORDER BY o.order_date DESC)as rnk
    FROM Orders o LEFT JOIN Customers c 
    ON (o.customer_id=c.customer_id)
)

SELECT name as customer_name, customer_id, order_id, order_date FROM cte
WHERE rnk <= 3
