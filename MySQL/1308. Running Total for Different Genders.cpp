...
Write an SQL query to find the Running total score for each gender on each day.
...
WITH cte1 as(
    SELECT gender, day, (SELECT SUM(s2.score_points) FROM Scores s2 WHERE s2.day<=s1.day AND gender='F') as total 
    FROM Scores s1
    WHERE gender='F'
    GROUP BY gender, day
    ORDER BY gender, day
), cte2 as(
    SELECT gender, day, (SELECT SUM(s2.score_points) FROM Scores s2 WHERE s2.day<=s1.day AND gender='M') as total 
    FROM Scores s1
    WHERE gender='M'
    GROUP BY gender, day
    ORDER BY gender, day
