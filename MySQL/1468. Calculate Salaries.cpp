...
SELECT company_id, employee_id, employee_name,
       (CASE WHEN max_salary>10000 THEN ROUND(0.51*salary)
             WHEN max_salary>1000 THEN ROUND(0.76*salary)
             ELSE salary END
       ) as salary
FROM cte
...
SELECT company_id, employee_id, employee_name,
       (CASE WHEN MAX(salary) OVER (PARTITION BY company_id)>10000 THEN ROUND(0.51*salary)
             WHEN MAX(salary) OVER (PARTITION BY company_id)>1000 THEN ROUND(0.76*salary)
             ELSE salary END
        ) as salary
FROM Salaries
