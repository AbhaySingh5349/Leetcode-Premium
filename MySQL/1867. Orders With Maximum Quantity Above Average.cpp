...
You are running an e-commerce site that is looking for imbalanced orders. An imbalanced order is one whose maximum quantity is strictly greater than the 
...
GROUP BY order_id
HAVING MAX(quantity)>(SELECT MAX(qty) FROM (SELECT AVG(quantity) as qty
                                               FROM OrdersDetails 
                                               GROUP BY order_id
                                           ) as o2
                     )
